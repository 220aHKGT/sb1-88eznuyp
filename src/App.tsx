      ...
      try {
        ...
        for (let i = 0; i < pagesToExport.length; i++) {
          const replacedTexts = texts.map(text => ({
            ...text,
            text: replacePlaceholders(text.text, pagesToExport[i])
          }));
            ...
            {texts.map((text) => (
              <div
                ...
                className={`absolute group ${
                  selectedId === text.id ? 'text-blue-600' : 'text-gray-800'
                }`}
                style={{
                  left: `${text.x}px`,
                  top: `${text.y}px`,
              ...
              >
                {!isExporting && (
                  <div className="flex items-center gap-1 mb-1">
                    ...
                    {text.isOverflowing && (
                      <AlertTriangle size={14} className="text-amber-500" title="Text is overflowing the canvas" />
                    )}
                  </div>
                )}
                <div
